...
in {
  options.nixarr.anchorr = {
    ...
    envFile = mkOption {
      ...
      description = "Environment file containing secrets like DISCORD_TOKEN, DISCORD_BOT_ID, DISCORD_GUILD_ID, TMDB_API_KEY, etc.";
    };
    ...
    systemd.services.anchorr-setup = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "anchorr-setup" ''
          ...
          if [[ -f '${if cfg.envFile != null then cfg.envFile else "/dev/null"}' ]]; then
            cat '${if cfg.envFile != null then cfg.envFile else "/dev/null"}' >> "$ENV_FILE"
          fi
