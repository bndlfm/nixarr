...
in {
  options.nixarr.anchorr = {
    ...
    envFile = mkOption {
      ...
      example = "/data/.secret/anchorr/env";
      description = ''
        Deprecated alias for [](#opt-nixarr.anchorr.secretsfile).
      '';
    };
  ...
  config = mkIf (nixarr.enable && cfg.enable) {
    warnings = optional (cfg.envFile != null) ''
      nixarr.anchorr.envFile is deprecated; use nixarr.anchorr.secretsFile instead.
    '';

    assertions = [
      {
        assertion = !(cfg.secretsFile != null && cfg.envFile != null);
        message = ''
          You cannot set both nixarr.anchorr.secretsFile and nixarr.anchorr.envFile.
          Please use nixarr.anchorr.secretsFile.
        '';
      }
      {
    ...
    systemd.services.anchorr-setup = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "anchorr-setup" ''
          ...
          if [[ -f '${if effectiveSecretsFile != null then effectiveSecretsFile else "/dev/null"}' ]]; then
            cat '${if effectiveSecretsFile != null then effectiveSecretsFile else "/dev/null"}' >> "$ENV_FILE"
          fi
