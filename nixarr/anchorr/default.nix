...
in {
  options.nixarr.anchorr = {
    ...
    secretsFile = mkOption {
      ...
      description = "Environment file containing secrets like DISCORD_TOKEN, TMDB_API_KEY, etc.";
    };
    ...
    systemd.services.anchorr-setup = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "anchorr-setup" ''
          ...
          if [[ -f '${cfg.secretsFile or "/dev/null"}' ]]; then
            cat '${cfg.secretsFile or "/dev/null"}' >> "$ENV_FILE"
          fi
